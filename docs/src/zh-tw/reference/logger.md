...
創建一個`Logger`，通常會綁定一個默認的`ConsoleAppender`。開發者通常應通過`LogManager.getLogger`獲取Logger。
...
获取本次应用启动至被调用时期间生成的日志（需對應Appender啟用歷史記錄）

## `getAllHistoryOfAppender(predicates)` <Badge type="tip" text="1.1.0 +" />

- `predicates` string - 用于搜索具名`FileAppender`

獲取指定具名文件類Appender的歷史日誌內容，通常包含已滾動生成的緩存文件（`DailyRollingFileAppender`還會包含當前會話，當前會話部分需啟用歷史記錄）
...
向`Logger`添加一個新的`FileAppender`，支持鏈式調用。注意：當 `useWorker` 為 `true` 時，`encryptor` 不會生效。
